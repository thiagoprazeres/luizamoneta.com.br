...
export function buildInstitutionalVideoPrompt({
  ...
  const promptParts = [
    ...
    'Distribuicao recomendada do tempo: abertura muito curta com a dor ou necessidade, transicao rapida para o cuidado domiciliar, e fechamento forte dedicado ao pre-atendimento com a Luiza IA.',
  ...
  if (hasClosingArt) {
    promptParts.push(
      'Existe uma arte final oficial de fechamento que sera aplicada fora do Sora na versao final deste video.',
      'Por isso, nao tente renderizar texto final longo, nao tente escrever CTA completo na ultima cena e nao dependa de lettering na tela para encerrar a mensagem.',
      'Reservar os 2 segundos finais para uma transicao limpa, estavel e elegante, com pouco movimento, preparando a entrada da arte final oficial do pre-atendimento.',
      'No ultimo plano gerado pelo Sora, priorizar composição calma, respiro visual, luz suave e continuidade de marca, para que a arte final possa entrar por corte ou fade sem parecer truncada.',
      hasReference
        ? 'Encerrar com composição final inspirada no simbolo da marca e com area visual limpa para a arte oficial de fechamento.'
        : 'Encerrar com composição final de marca e com area visual limpa para a arte oficial de fechamento.'
    );
  } else {
    promptParts.push(
      ...
      'Sugestao de tela final: "Comece seu pre-atendimento com a Luiza IA" e em seguida "Atendimento domiciliar em Recife".',
      hasReference
